...
char* WD() {
    char* appDataPath = getenv("PROGRAMFILES");

    if (appDataPath != NULL) {
        char* fullPath[512];
        snprintf(*fullPath, sizeof(fullPath), "%s\\Renderlabs\\Studio", appDataPath);
        return *fullPath;
    } else {
        printf("Failed to get PROGRAMFILES environment variable.\n");
        exit(1);
    }
